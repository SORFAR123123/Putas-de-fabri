...
// ====================
// NUEVO: SISTEMA DE VISTA PREVIA (HOVER PREVIEW)
// ====================

// Variables para controlar el preview
let previewTimer = null;
let currentPreviewElement = null;

// Configuración de previews para cada tipo de contenido
...
function crearElementoPreview() {
    ...
    previewDiv.innerHTML = `
        <div id="preview-image" style="width: 100%; height: 100%; background-size: cover; background-position: center;"></div>
        <div id="preview-info" style="position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(transparent, rgba(0,0,0,0.9)); color: white; padding: 15px; font-size: 14px; display: none;">
            <div id="preview-title" style="font-weight: bold; margin-bottom: 5px;"></div>
            <div id="preview-desc" style="font-size: 12px; opacity: 0.8;"></div>
        </div>
    `;
...
// Obtener datos del preview según el tipo de elemento
function obtenerDatosPreview(elemento, tipo) {
    const datos = {
        imagen: '',
        titulo: '',
        descripcion: ''
    ...
    if (elemento.classList.contains('contenedor-item')) {
        const imgDiv = elemento.querySelector('.contenedor-img');
        const tituloDiv = elemento.querySelector('.contenedor-numero');
        const descP = elemento.querySelector('p');
        
        datos.imagen = imgDiv ? window.getComputedStyle(imgDiv).backgroundImage.slice(5, -2) : '';
        datos.titulo = tituloDiv ? tituloDiv.textContent : 'Contenedor';
        datos.descripcion = descP ? descP.textContent : '';
    }
    // Verificar si es un subcontenedor
    else if (elemento.classList.contains('subcontenedor-item')) {
        const imgDiv = elemento.querySelector('.subcontenedor-img');
        const tituloH3 = elemento.querySelector('h3');
        const descP = elemento.querySelector('p');
        
        datos.imagen = imgDiv ? window.getComputedStyle(imgDiv).backgroundImage.slice(5, -2) : '';
        datos.titulo = tituloH3 ? tituloH3.textContent : 'Subcontenedor';
        datos.descripcion = descP ? descP.textContent : '';
    }
    // Verificar si es un mazo
    else if (elemento.classList.contains('mazo-item')) {
        const tituloH3 = elemento.querySelector('h3');
        const descP = elemento.querySelector('p');
        
        // Usar imagen por defecto para mazos
        datos.imagen = 'https://images.unsplash.com/photo-1544717305-2782549b5136?w=400&h=300&fit=crop';
        datos.titulo = tituloH3 ? tituloH3.textContent : 'Mazo';
        datos.descripcion = descP ? descP.textContent : '10 palabras japonesas';
    }
    // Verificar si es una tarjeta del menú principal
    else if (elemento.classList.contains('card')) {
        const imgDiv = elemento.querySelector('.card-icon');
        const tituloH3 = elemento.querySelector('h3');
        const descP = elemento.querySelector('p');
        
        if (imgDiv && imgDiv.textContent.includes('📚')) {
            datos.imagen = 'https://images.unsplash.com/photo-1544717305-2782549b5136?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('🎬')) {
            datos.imagen = 'https://images.unsplash.com/photo-1574375927938-d5a98e8ffe85?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('🎌')) {
            datos.imagen = 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('🎵')) {
            datos.imagen = 'https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('🎧')) {
            datos.imagen = 'https://images.unsplash.com/photo-1572860177022-8fda92a90b95?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('🎮')) {
            datos.imagen = 'https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('⚔️')) {
            datos.imagen = 'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('🎯')) {
            datos.imagen = 'https://images.unsplash.com/photo-1536922246289-88c42f957773?w=400&h=300&fit=crop';
        } else if (imgDiv && imgDiv.textContent.includes('🔄')) {
            datos.imagen = 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=400&h=300&fit=crop';
        } else {
            datos.imagen = 'https://images.unsplash.com/photo-1518709268805-4e9042af2176?w=400&h=300&fit=crop';
        }
        
        datos.titulo = tituloH3 ? tituloH3.textContent : 'Sección';
        datos.descripcion = descP ? descP.textContent : '';
    }
...
function determinarTipoElemento(elemento) {
    if (elemento.classList.contains('contenedor-item')) return 'contenedor';
    if (elemento.classList.contains('subcontenedor-item')) return 'subcontenedor';
    if (elemento.classList.contains('mazo-item')) return 'mazo';
    if (elemento.classList.contains('card')) return 'contenedor';
    return 'default';
}

// Mostrar preview
function mostrarPreview(event, elemento) {
    // Limpiar timer anterior
    if (previewTimer) {
        clearTimeout(previewTimer);
    }
    
    currentPreviewElement = elemento;
    
    // Iniciar timer para mostrar después del delay
    previewTimer = setTimeout(() => {
        const tipo = determinarTipoElemento(elemento);
        const config = previewConfigs[tipo] || previewConfigs['contenedor'];
        const datos = obtenerDatosPreview(elemento, tipo);
        
        // Obtener o crear elemento preview
        let previewDiv = document.getElementById('hover-preview');
        if (!previewDiv) {
            previewDiv = crearElementoPreview();
        }
        
        // Configurar tamaño
        previewDiv.style.width = config.width + 'px';
        previewDiv.style.height = config.height + 'px';
        
        // Configurar imagen y texto
        const imgDiv = previewDiv.querySelector('#preview-image');
        const infoDiv = previewDiv.querySelector('#preview-info');
        const tituloDiv = previewDiv.querySelector('#preview-title');
        const descDiv = previewDiv.querySelector('#preview-desc');
        
        imgDiv.style.backgroundImage = `url('${datos.imagen || 'https://images.unsplash.com/photo-1518709268805-4e9042af2176?w=400&h=300&fit=crop'}')`;
        
        if (datos.titulo || datos.descripcion) {
            tituloDiv.textContent = datos.titulo;
            descDiv.textContent = datos.descripcion.length > 60 ? datos.descripcion.substring(0, 60) + '...' : datos.descripcion;
        ...
        switch(config.position) {
            case 'top':
                top = rect.top - config.height - 15;
                left = rect.left + (rect.width / 2) - (config.width / 2);
                break;
            case 'top-right':
                top = rect.top - config.height - 15;
                left = rect.right - config.width;
                break;
            case 'right':
                top = rect.top + (rect.height / 2) - (config.height / 2);
                left = rect.right + 15;
                break;
            case 'left':
                top = rect.top + (rect.height / 2) - (config.height / 2);
                left = rect.left - config.width - 15;
                break;
            default:
                top = rect.top - config.height - 15;
                left = rect.left;
        ...
        if (left + config.width > viewportWidth - 10) left = viewportWidth - config.width - 10;
        if (left < 10) left = 10;
        if (top + config.height > viewportHeight - 10) top = viewportHeight - config.height - 10;
        
        previewDiv.style.top = Math.max(10, top) + 'px';
        previewDiv.style.left = Math.max(10, left) + 'px';
...
function ocultarPreview() {
    ...
    const previewDiv = document.getElementById('hover-preview');
    if (previewDiv) {
...
// Inicializar sistema de preview en elementos
function inicializarSistemaPreview() {
    console.log('👁️ Inicializando sistema de vista previa...');
    
    // Eliminar listeners antiguos
    document.removeEventListener('mouseover', manejarMouseOver);
    document.removeEventListener('mouseout', manejarMouseOut);
    
    // Agregar nuevos listeners
    document.addEventListener('mouseover', manejarMouseOver);
    document.addEventListener('mouseout', manejarMouseOut);
    
    // Crear elemento preview si no existe
    if (!document.getElementById('hover-preview')) {
        crearElementoPreview();
    }
    
    console.log('✅ Sistema de vista previa activado');
}

// Manejar mouse over
function manejarMouseOver(event) {
    const elemento = event.target;
    
    // Verificar si el elemento o sus padres tienen las clases para preview
    const elementoConPreview = elemento.closest('.contenedor-item, .subcontenedor-item, .mazo-item, .card');
    
    if (elementoConPreview && elementoConPreview !== currentPreviewElement) {
        mostrarPreview(event, elementoConPreview);
    }
}
...
function manejarMouseOut(event) {
    ...
    if (elementoConPreview) {
        // Verificar si el mouse se movió a otro elemento con preview
        const relatedTarget = event.relatedTarget;
        const nuevoElementoConPreview = relatedTarget ? relatedTarget.closest('.contenedor-item, .subcontenedor-item, .mazo-item, .card') : null;
        
        if (!nuevoElementoConPreview || nuevoElementoConPreview !== elementoConPreview) {
            ocultarPreview();
        }
    }
}

// Función para actualizar previews después de cargar contenido dinámico
function actualizarPreviewsDinamicos() {
    setTimeout(() => {
        console.log('🔄 Actualizando previews para contenido dinámico');
        // Los event listeners globales ya manejarán los nuevos elementos
    }, 500);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar sistema de vista previa
    setTimeout(() => {
        inicializarSistemaPreview();
        console.log('👁️ Sistema de vista previa inicializado');
    }, 1000);
    ...
    // Observador para actualizar previews cuando se carga contenido dinámico
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.addedNodes.length > 0) {
                actualizarPreviewsDinamicos();
            }
        });
    });
