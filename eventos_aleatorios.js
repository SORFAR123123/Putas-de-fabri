 ...
 * Se dispara cada 2-5 mensajes del usuario (aleatorio).
 */
let quintContadorEventos = 0;
let quintProximoEventoEn = 0;

function quintIniciarTimerEventos() {
    // Elegir cuándo será el próximo evento (aleatorio entre 2 y 5 turnos)
    quintProximoEventoEn = Math.floor(Math.random() * 4) + 2; // 2-5
    quintContadorEventos = 0;
}

function quintContarTurnoEvento() {
    quintContadorEventos++;
    if (quintContadorEventos >= quintProximoEventoEn) {
        quintDispararEventoAleatorio();
        quintProximoEventoEn = Math.floor(Math.random() * 4) + 2; // Reset: próximo en 2-5 turnos
        quintContadorEventos = 0;
    }
}
