...
const HoraDelDia = {
    ...
    _aplicarFondoVisual(período) {
        ...
        // Si hay imagen configurada, usarla
        if (imagenUrl && imagenUrl.trim() !== "") {
            const fondo = `url('${imagenUrl}') center/cover no-repeat fixed`;
            if (chat) {
                chat.style.background = fondo;
                chat.style.backgroundAttachment = "fixed";
            }
            // Overlay oscuro para legibilidad
            if (app) {
                app.style.setProperty("--hora-overlay", "rgba(0,0,0,0.3)");
            }
        } else {
            // Usar gradiente
            if (chat) {
                chat.style.background = gradiente;
                chat.style.backgroundAttachment = "fixed";
            }
            if (app) {
                app.style.setProperty("--hora-overlay", "transparent");
            }
        }
    },
...
function horaInjectStyles() {
    ...
    style.textContent = `
        ...
        /* OVERLAY DE FONDO (para legibilidad) */
        #quint-chat-mensajes {
            position: relative;
            transition: background 1s ease;
        }
        #quint-chat-mensajes::before {
            content: ''; position: absolute; inset: 0;
            background: var(--hora-overlay, transparent);
            pointer-events: none; z-index: 0;
            transition: background 1s ease;
        }
        #quint-chat-mensajes > * { position: relative; z-index: 2; }
